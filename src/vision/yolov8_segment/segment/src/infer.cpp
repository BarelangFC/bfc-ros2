...
void YoloDetector::draw_image(cv::Mat& img, std::vector<Detection>& inferResult) {
    //std::vector<cv::Point> fieldPolyline; // Untuk menyimpan polyline lapangan
    cv::Rect ballBoundingBox;             // Untuk menyimpan bounding box bola
    cv::Point ballCenter;                 // Untuk menyimpan titik tengah bola
    bool ballDetected = false;            // Untuk menandai apakah bola terdeteksi
    //bool fieldDetected = false;           // Untuk menandai apakah lapangan terdeteksi
    //bool ballInsideField = false;         // Untuk menandai apakah bola berada di dalam lapangan
    //bool drawBallMask = true;  	   // Untuk menentukan apakah mask bola perlu digambar
    //bool useFieldForBallCheck = false;     // Untuk menentukan apakah class 'field' digunakan untuk pengecekan bola

    // Proses deteksi hasil inferensi
    for (size_t i = 0; i < inferResult.size(); i++) {
        int classId = inferResult[i].classId;

        /* if (classId == 1) {  // Field
            fieldDetected = true;

            // Convert mask to polyline
            cv::Mat mask = cv::Mat(img.size(), CV_32F, inferResult[i].maskMatrix.data());
            ...
            if (!contours.empty()) {
                // Offset the contour slightly
                std::vector<cv::Point> offsetContour = contours[0];
                for (auto& pt : offsetContour) {
                    pt.x += 0;  // Offset horizontal
                    pt.y += 0;  // Offset vertikal
                }
                fieldPolyline = offsetContour;
            }
        } else*/ if (classId == 0) {  // Ball
            ballDetected = true;

            // Use bounding box only
            ballBoundingBox = cv::Rect(
                cv::Point(round(inferResult[i].bbox[0]), round(inferResult[i].bbox[1])),
                cv::Point(round(inferResult[i].bbox[2]), round(inferResult[i].bbox[3]))
            );
            ballCenter = (ballBoundingBox.br() + ballBoundingBox.tl()) * 0.5;
            cv::circle(img, ballCenter, 4, kBallColor, -1);

            // Check if ball's center is inside the field
            /*if (useFieldForBallCheck && !fieldPolyline.empty()) {
                ballInsideField = cv::pointPolygonTest(fieldPolyline, ballCenter, false) >= 0;
            }*/
        }
    }

    /*// Gambar hasil berdasarkan kondisi
    if (!fieldDetected && !ballDetected) {
        // Jika tidak ada deteksi sama sekali
        return;
    } else if (!fieldDetected && ballDetected) {
        // Hanya gambar bounding box bola
        //cv::rectangle(img, ballBoundingBox, kBallColor, 1);
        cv::circle(img, ballCenter, 2, kBallColor, -1);
    } else if (fieldDetected && !ballDetected) {
        // Hanya gambar polyline lapangan
        if (!fieldPolyline.empty()) {
            cv::polylines(img, fieldPolyline, true, kFieldColor, 2);
        }
    } else if (fieldDetected && ballDetected) {
        // Gambar polyline lapangan
        if (!fieldPolyline.empty()) {
            cv::polylines(img, fieldPolyline, true, kFieldColor, 2);
        }

        if (useFieldForBallCheck) {
            if (ballInsideField) {
                // Gambar bola jika di dalam lapangan
                //cv::rectangle(img, ballBoundingBox, kBallColor, 1);
                cv::circle(img, ballCenter, 2, kBallColor, -1);
                cv::polylines(img, fieldPolyline, true, kFieldColor, 2);
            }
        } else {
            // Gambar bola tanpa pengecekan lapangan
            //cv::rectangle(img, ballBoundingBox, kBallColor, 1);
            //cv::circle(img, ballCenter, 2, kBallColor, -1);
        }
    }*/
    ...
    /*if (!fieldDetected && !ballDetected) {
        ...
        // Hanya gambar bounding box bola
        //cv::rectangle(img, ballBoundingBox, kBallColor, 2);
        cv::circle(img, ballCenter, 4, kBallColor, -1);
